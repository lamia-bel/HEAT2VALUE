...
if __name__ == "__main__":
    ...
    df = generer_simulation()

    df_decision, HRR, reutilisee, stockee, rejetee = heat2value(df)

    print(df_decision.head(20).to_string(index=False))

    print()

    print(f"Chaleur totale produite  : {df['chaleur_produite_kW'].sum()*0.25:.0f} kWh")
    print(f"Chaleur réutilisée       : {reutilisee:.0f} kWh")
    print(f"Chaleur stockée          : {stockee:.0f} kWh")
    print(f"Chaleur rejetée          : {rejetee:.0f} kWh")
    print(f"HRR                      : {HRR:.2f}%")